...
import { MoreHorizontal, Plus, ArrowUp, ArrowDown, ArrowUpDown } from "lucide-react"
...
export default function MonitorsTable() {
  ...
  const sorted = useMemo(() => {
    ...
    const arr = data
      .filter((m) => {
        if (!q) return true
        return m.name.toLowerCase().includes(q) || m.endpoint.toLowerCase().includes(q)
      })
      .slice()
    ...
    arr.sort((a, b) => {
      ...
      switch (sortKey) {
        ...
        case "p95":
          ...
          break
        case "lastCheck":
          cmp = parseLastCheckToMinutes(a.lastCheck) - parseLastCheckToMinutes(b.lastCheck)
          break
      }
      ...
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            />
            <Button className="gap-2" variant="default" onClick={() => setAddOpen(true)}>
          ...
          <TableHeader>
            <TableRow>
              ...
              <TableHead
                ...
                aria-sort={sortKey === "endpoint" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
              >
              ...
              <TableHead aria-sort={sortKey === "uptime" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}>
                <HeaderSortButton label="Uptime" column="uptime" />
              </TableHead>
              <TableHead aria-sort={sortKey === "p95" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}>
                <HeaderSortButton label="p95" column="p95" />
              </TableHead>
              <TableHead
                className="hidden md:table-cell"
                aria-sort={sortKey === "lastCheck" ? (sortDir === "asc" ? "ascending" : "descending") : "none"}
              >
          ...
          <TableBody>
            {sorted.map((m) => (
              <TableRow key={m.id}>
                ...
                <TableCell className="hidden md:table-cell text-muted-foreground">{m.endpoint}</TableCell>
                <TableCell>
                  <div className="flex items-center gap-2">
                    <span
                      className={
                ...
                <TableCell className="hidden md:table-cell text-muted-foreground">{m.lastCheck}</TableCell>
