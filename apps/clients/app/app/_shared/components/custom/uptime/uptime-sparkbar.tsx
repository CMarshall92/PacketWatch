...
export default function UptimeSparkbar({
  ...
  return (
    <div className="w-full">
      <svg viewBox={`0 0 ${width} ${height}`} className="w-full h-[80px]" role="img" aria-label="Uptime spark bars">
        {/* Baseline at 100% */}
        ...
        {data.map((d, i) => {
          ...
          const y = height - h
          const color = d.uptime >= 99.9 ? "hsl(142 71% 45%)" : d.uptime >= 99.5 ? "hsl(38 92% 50%)" : "hsl(0 84% 60%)"
          return <rect key={d.label} x={x} y={y} width={barWidth} height={h} rx={2} ry={2} fill={color} />
        })}
      </svg>
    </div>
  )
