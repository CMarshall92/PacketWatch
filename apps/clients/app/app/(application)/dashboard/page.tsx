...
export default function Page() {
  ...
  const [confirmedRegionRows, setConfirmedRegionRows] = useState<{ code: string; latency: number; status: Status }[]>(
    () => {
      const code = "eu-west"
  ...
  function toggleRegion(code: string, checked: boolean) {
    setSelectedRegions((prev) => (checked ? Array.from(new Set([...prev, code])) : prev.filter((c) => c !== code)))
  }
      ...
      <section className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="lg:col-span-2">
          ...
          <CardContent className="space-y-2">
            <div className="text-2xl font-semibold">{formatPct(overview.uptimePct)}</div>
            <div className="text-sm text-muted-foreground">Target 99.95% SLO</div>
            <div className="mt-2">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-lg font-medium">
              ...
              {overview.status === "degraded" && <span className="text-amber-700">Degraded performance</span>}
              {overview.status === "outage" && <span className="text-red-700">Major outage</span>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-semibold">{formatMs(overview.avgRespMs)}</div>
            <div className="text-sm text-muted-foreground">p95 typically ~420ms</div>
          </CardContent>
        </Card>
      </section>
          ...
          <CardContent className="space-y-3 flex-1">
            {confirmedRegionRows.length > 0 ? (
              confirmedRegionRows.map((r) => {
                ...
                return (
                  <div key={r.code} className="flex items-center justify-between rounded-md border p-3">
                    ...
                    <div className="text-sm text-muted-foreground">{formatMs(r.latency)}</div>
                  </div>
                )
              })
            ) : (
              <div className="text-xs text-muted-foreground">No regions selected.</div>
            )}
          </CardContent>

          <CardFooter className="pt-2 mt-auto">
            <Button className="w-full bg-transparent" variant="outline" onClick={openRegionsModal}>
              {confirmedRegionRows.length > 0 ? "Manage regions" : "Select regions"}
            </Button>
          </CardFooter>
        </Card>
      </section>
      ...
      <Dialog open={regionsOpen} onOpenChange={setRegionsOpen}>
        <DialogContent className="sm:max-w-lg">
          <DialogHeader>
            ...
            <DialogDescription>Choose one or more regions to run pings from.</DialogDescription>
          </DialogHeader>
          ...
          <p className="text-xs text-muted-foreground">At least one region must remain selected.</p>

          <DialogFooterBase className="gap-2">
            <Button variant="ghost" onClick={() => setRegionsOpen(false)}>
              Cancel
            </Button>
            <Button onClick={confirmRegions} disabled={selectedRegions.length === 0}>
              Confirm pings from {selectedRegions.length > 0 ? `${selectedRegions.length} selected` : "regions"}
            </Button>
          </DialogFooterBase>
        </DialogContent>
      </Dialog>
...
function makeUptimeSeries(range: RangeKey) {
  ...
  return Array.from({ length: n }).map((_, i) => {
    ...
    return { label: range === "24h" ? `${i}:00` : `D${i + 1}`, uptime: +pct.toFixed(3) }
  })
}
